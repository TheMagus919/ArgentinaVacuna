    ...
    try {
        ...
        if(req.body.dniPaciente != "" || req.body.dniPaciente != null && req.body.dniAgente != "" || req.body.dniAgente != null && req.body.nroLote != "" || req.body.nroLote != null && req.body.fechaDeAplicacion != "" || req.body.fechaDeAplicacion != null && req.body.idCentro!= "" || req.body.idCentro != null){
            ...
            if(fecha.date() > fechaActual.date() && fecha.month()>= fechaActual.month() && fecha.year() >= fechaActual.year()){
                ...
                if(fecha.date() > fechaVen.date()  && fecha.month() >= fechaVen.month() && fecha.year() >= fechaVen.year()){
                    res.render("error", {message:"Bad Request",error:{status:400,stack:"No se puede aplicar vacunas vencidas."}});
                }else{
                    ...
                    .then((result)=>{
                        if(result==0){
                            ...
                            DistribucionCentro.findOne({where:[{idCentro:centro},{nroLote:req.body.nroLote},{cantidadDeVacunas:{[Op.gt]:0}}]})
                            .then((resultado)=>{
                                if(resultado==null || resultado.length == 0){
                                    res.render("error", {message:"Bad Request",error:{status:400,stack:"Error al crear registro de Aplicacion."}});
                                }else{
                                    const cantidad = parseInt(resultado.cantidadDeVacunas);
                                    cantidad = cantidad - 1;
                                    DistribucionCentro.update(({idDisCentro:resultado.idDisCentro, nroLote:resultado.nroLote, cantidadDeVacunas:cantidad, idDepProv:resultado.idDepProv, fechaDeSalidaDepProv:resultado.fechaDeSalidaDepProv, idCentro:resultado.idCentro, fechaLlegadaCentro:resultado.fechaLlegadaCentro},{where:{idDisCentro: resultado.idDisCentro}}))
                                    .then((resulta)=>{
                                        res.redirect("/aplicacion");
                                    })
                                    .catch((err) => res.render("error", {error:err}));
...
exports.mostrarVistaListarPacientesXTipoVacuna = async function(req, res){
    const tipoVacuna = await TipoVacuna.findAll();
    const centrosProv = await CentroDeVacunacion.findAll({
        attributes: ['provincia'],
        group: [['provincia', 'ASC']]
    });
    const centrosLoc = await CentroDeVacunacion.findAll({
        attributes: ['localidad'],
        group: [['localidad', 'ASC']]
    });
    res.render("aplicacion/formXTipoVacuna",{listaVacuna:tipoVacuna, listaProv:centrosProv, listaLoc:centrosLoc, title:"Aplicacion"});
};
    ...
    try {
        Aplicacion.findAll({
            include: [
                ...
                {
                    model: CentroDeVacunacion,
                    where:{provincia:req.body.provincia,localidad:req.body.localidad},
                    attributes: ['provincia', 'localidad']
                },
                {
                    model: Paciente,
            ...
            where:[{'$CentroDeVacunacion.provincia$':req.body.provincia,
                    '$CentroDeVacunacion.localidad$':req.body.localidad,
                    '$LoteProveedor.TipoVacuna.nombre$':req.body.nombre
            }],
            attributes: [
                [sequelize.col('LoteProveedor.TipoVacuna.nombre'), 'tipoVacuna'],
                [sequelize.fn('COUNT', sequelize.col('Paciente.dniPaciente')), 'cantidadPacientes']
...
exports.mostrarVistaListarPacientesXVacunaVencidas = async function(req, res){
    const tipoVacuna = await TipoVacuna.findAll();
    const centrosProv = await CentroDeVacunacion.findAll({
        attributes: ['provincia'],
        order: [['provincia', 'ASC']]
    ...
    res.render("aplicacion/formXVacunaVencida",{listaVacuna:tipoVacuna, listaCentros:centros, listaProv:centrosProv,title:"Aplicacion"});
};
    ...
    try {
        Aplicacion.findAll({
          include: [
            {
              model: Paciente,
              attributes: ['nombre', 'apellido', 'dniPaciente']
            },
            {
              model: CentroDeVacunacion,
              where: { provincia:req.body.provincia, idCentro:req.body.idCentro },
              attributes: ['nombre', 'provincia']
            },
            {
              model: LoteProveedor,
          ...
          where:[{
                '$CentroDeVacunacion.provincia$':req.body.provincia,
                '$CentroDeVacunacion.idCentro$':req.body.idCentro,
                '$LoteProveedor.TipoVacuna.nombre$':req.body.nombre,
                '$LoteProveedor.estado$':'vencidas'
          }]
        })
        .then((result)=>{
            if(result == null || result.length === 0){
                ...
                const resultados =result.map(aplicacion => ({
                pacienteDNI: aplicacion.Paciente.dniPaciente,
                pacienteNombre: aplicacion.Paciente.nombre+" "+aplicacion.Paciente.apellido,
                centro: aplicacion.CentroDeVacunacion.nombre,
                provincia: aplicacion.CentroDeVacunacion.provincia,
                tipoVacuna: aplicacion.LoteProveedor.TipoVacuna.nombre
                }));
